...
function ProgressLine({ currentIndex }) {
  const max = STEPS.length - 1
  const safeIndex = Math.min(Math.max(currentIndex, 0), max)
  const progressPct = max === 0 ? 0 : (safeIndex / max) * 100
  const scaleX = max === 0 ? 0 : safeIndex / max
  ...
  return (
    <div className="relative">
      ...
      <div
        className="absolute left-2.5 right-2.5 top-2.5 h-0.5 origin-left bg-[var(--active)]"
        ...
        {STEPS.map((step, index) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`mt-1 text-[11px] leading-tight whitespace-nowrap text-center ${
