...
function normalizeDiscountType(value) {
  const key = String(value ?? "")
    .trim()
    .toUpperCase();
  if (key === "FIXED" || key === "FIXED_AMOUNT" || key === "AMOUNT")
    return "fixed";
...
function mapPromoCodeItem(item) {
  ...
  const minOrderAmount =
    item.minOrderAmount ?? item.minOrder ?? item.minimumOrder ?? 0;
  const isActive =
    item.isActive != null
  ...
  return {
    ...
    usedCount:
      Number(item.usedCount ?? item.used ?? item.redemptions ?? 0) || 0,
    status: toPromoCodeStatus(
      item.status,
      isActive,
      item.expiryDate ?? item.expiresAt,
    ),
    expiryDate: formatDateValue(
      item.expiryDate ?? item.expiresAt ?? item.expirationDate,
    ),
    isActive,
    raw: item,
  };
...
export function buildPromoCodePayload(form = {}) {
  ...
  const payload = {
    ...
    ...(form.applicableUsers
      ? { applicableUsers: String(form.applicableUsers).toUpperCase() }
      : {}),
  };
