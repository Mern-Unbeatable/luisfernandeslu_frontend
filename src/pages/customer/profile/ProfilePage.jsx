...
} from '@/features/customer/customerProfileApi'
import {
  emptyCustomerProfileForm,
  mapCustomerAddressToPayload,
  mapCustomerProfileToForm,
  mapCustomerProfileToPayload,
} from '@/features/customer/customerProfileMappers'
...
export default function ProfilePage() {
  ...
  const [form, setForm] = useState(emptyCustomerProfileForm)
  ...
  useEffect(() => {
    ...
    setForm(mapCustomerProfileToForm(data.profile))
  }, [data?.profile])
  ...
  const handleUpdateProfile = async (payload) => {
    ...
    try {
      const result = await updateProfile(mapCustomerProfileToPayload(payload))
        .unwrap()
  ...
  const handleSaveBillingAddress = async (address) => {
    ...
    try {
      const result = await updateBillingAddress(
        mapCustomerAddressToPayload(address),
      ).unwrap()
  ...
  const handleUploadAvatar = async (file) => {
    ...
    try {
      ...
      if (err?.message === 'invalid avatar type' || err?.message === 'avatar upload failed') {
        throw err
      }
