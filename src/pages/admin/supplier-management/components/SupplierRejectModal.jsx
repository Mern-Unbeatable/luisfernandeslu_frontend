...
const I18N_KEY = 'adminSupplierManagement'

export default function SupplierRejectModal({ open, supplierId, onClose, onRejected }) {
  const { t } = useTranslation()
  const [reason, setReason] = useState('')
  const [invalidDocuments, setInvalidDocuments] = useState([])

  const { data, isLoading } = useGetAdminSupplierByIdQuery(supplierId, {
    skip: !open || !supplierId,
  })

  const [rejectSupplier, { isLoading: isSubmitting }] = useRejectAdminSupplierMutation()
    ...
    >
      ...
      <div className="relative z-10 max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl border border-gray-200 bg-white p-6 shadow-xl sm:p-8">
        ...
        <h2 id={titleId} className="pr-8 text-lg font-bold text-[var(--primary-text)]">
          {t(`${I18N_KEY}.rejectModal.title`, { name: supplier?.name || '—' })}
        </h2>
