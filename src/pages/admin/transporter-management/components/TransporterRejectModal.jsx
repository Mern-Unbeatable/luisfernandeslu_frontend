...
export default function TransporterRejectModal({
  open,
  transporterId,
  onClose,
  onRejected,
  ...
  const { data, isLoading } = useGetAdminTransporterByIdQuery(transporterId, {
    skip: !open || !transporterId,
  })

  const [rejectTransporter, { isLoading: isSubmitting }] =
    useRejectAdminTransporterMutation()
  ...
  if (!open || !transporterId) return null
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const result = await rejectTransporter({
        transporterId,
    ...
    >
      ...
      <div className="relative z-10 max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl border border-gray-200 bg-white p-6 shadow-xl sm:p-8">
        ...
        <h2 id={titleId} className="pr-8 text-lg font-bold text-[var(--primary-text)]">
          {t(`${I18N_KEY}.rejectModal.title`, { name: transporter?.name || '—' })}
        </h2>
