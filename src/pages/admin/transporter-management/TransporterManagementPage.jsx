...
import {
  ADMIN_TRANSPORTERS,
  ADMIN_TRANSPORTER_STATS,
  ADMIN_TRANSPORTER_TABS,
  filterTransportersBySearch,
  filterTransportersByTab,
...
export default function TransporterManagementPage() {
  ...
  const [detailTransporter, setDetailTransporter] = useState(null)
  const [rows] = useState(ADMIN_TRANSPORTERS)
  ...
  const filteredRows = useMemo(() => {
    const byTab = filterTransportersByTab(rows, activeTab)
    return filterTransportersBySearch(byTab, searchQuery)
    ...
    () => [
      {
        ...
        variant: 'primary',
        onClick: (row) => setDetailTransporter(row),
      },
      {
        id: 'suspend',
  ...
  const statCards = [
    {
      ...
      value: ADMIN_TRANSPORTER_STATS.totalTransporters,
    },
    {
      label: t(`${I18N_KEY}.stats.active`),
      value: ADMIN_TRANSPORTER_STATS.active,
    },
    {
      label: t(`${I18N_KEY}.stats.underReview`),
      value: ADMIN_TRANSPORTER_STATS.underReview,
    },
    {
      label: t(`${I18N_KEY}.stats.suspended`),
      value: ADMIN_TRANSPORTER_STATS.suspended,
    },
  ]
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <SupplierDetailsModal
        ...
        open={Boolean(detailTransporter)}
        supplier={detailTransporter}
        onClose={() => setDetailTransporter(null)}
      />
    </div>
  )
}
