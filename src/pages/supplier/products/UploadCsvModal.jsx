...
export default function UploadCsvModal({
  ...
  const formatError = (item) => {
    if (item.columns?.length) {
      return t(`panel.supplierProducts.csv.${item.messageKey}`, {
  ...
  const handleQueueImport = async () => {
    ...
    try {
      ...
      if (!result.ok) {
        setError(formatError(result.errors[0] || { messageKey: 'uploadFailed' }))
        return
      }
      ...
      setError(t('panel.supplierProducts.csv.uploadFailed'))
    } finally {
      setQueueing(false)
    }
  }
