...
import { getApiErrorMessage, toSelectOptions } from '@/features/supplier/apiError'
import {
...
export default function AddProductPage() {
  ...
  const { data: product, isLoading: productLoading, isError: productMissing } =
    useGetSupplierProductByIdQuery(productId, { skip: !isEdit })

  const { data: categories = [] } = useGetCategoriesQuery()
  const { data: subCategories = [] } = useGetSubcategoriesQuery(form.categoryId, {
    skip: !form.categoryId,
  })
  const { data: productTypes = [] } = useGetProductTypesQuery(form.subCategoryId, {
  ...
  const [createProduct, { isLoading: creating }] = useCreateSupplierProductMutation()
  const [updateProduct, { isLoading: updating }] = useUpdateSupplierProductMutation()
  ...
  const categoryOptions = useMemo(
    () => [
      { value: '', label: t('panel.supplierProducts.add.selectCategory', {
  ...
  const subCategoryOptions = useMemo(
    () => [
      { value: '', label: t('panel.supplierProducts.add.selectSubCategory', {
  ...
  const productTypeOptions = useMemo(
    () => [
      { value: '', label: t('panel.supplierProducts.add.selectProductType', {
  ...
  const handleSubmit = async (nextForm) => {
    ...
    const formData = buildProductFormData(
      {
        ...nextForm,
        warehouseLocationLabel: resolveWarehouseLabel(nextForm.warehouseLocation),
      },
  ...
  return (
    <>
      ...
      {isEdit && productLoading && hydratedId !== productId ? (
